...
export default class Vote {
    ...
    initiator: User;
    type: VoteType;
    votes: BetterMap<Player, boolean>;
    createdAt: Date;
    cancelled: boolean;
    ...
    constructor(
        ...
        votes: BetterMap<Player, boolean> = new BetterMap(),
        createdAt = new Date(),
        cancelled = false
    ) {
        this.ingame = ingame;
    ...
    serializeToClient(): SerializedVote {
        return {
            ...
            votes: this.votes.entries.map(([p, v]) => [p.user.id, v]),
    ...
    static deserializeFromServer(ingame: IngameGameState, data: SerializedVote): Vote {
        ...
        const votes = new BetterMap(data.votes.map(([uid, vote]) => {
            const player = ingame.players.get(ingame.entireGame.users.get(uid));
            return [player, vote];
        }));
